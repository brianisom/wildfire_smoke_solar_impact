...
subtitle: "Finding Balancing Authority Centroids"
output: 
...
EPA AQS Dictionary: https://aqs.epa.gov/aqsweb/documents/AQS_Data_Dictionary.html#:~:text=AQI,health%20concerns%20with%20elevated%20levels.
...
```{r, find geographic centroid}
plant_info <- read.csv("~/data/processed/solar_char_final.csv",
                        header = TRUE,
                        sep = ",")
...
```{r, find geographic weighted centroid}
...
consolidatedBApanel <- dailyBApanel %>%
  filter(., respondent %in% c("AVA","BPAT","BANC","EPE","IID","SRP"))

BANCpanel <- dailyBApanel %>%
  filter(., respondent %in% c("BANC"))
```

```{r}
plant_centroid_w$ID <- apply(plant_centroid_w, 1, function(row) call_geolocator_latlon(row['latitude'], row['longitude']))
...
census.pm <- readRDS("~/Documents/Research/smoke_and_solar/data/raw/smokePM2pt5_predictions_daily_tract_20060101-20201231.rds")
...
dailyBApanel <- dailyBApanel %>% mutate(
  smokePM_pred = ifelse(is.na(smokePM_pred),0,smokePM_pred)) %>% mutate(
  pm_pred = ifelse(Clear == 1, 0, smokePM_pred))
